...
if [ "${DEPLOY_KUBERNETES_BOOLEAN}" == true ]; then
   ...
   # Create a cluster calles mycluster with just a single server node:
    ...
    echo "#################### Create cluster"
    # sudo runuser -l ubuntu -c "k3d cluster create cluster-fg-ic-conn-1 --k3s-arg '--disable=traefik@server:*' --port '80:80@server:*' --port '443:443@server:*' --wait --timeout '300s'"
